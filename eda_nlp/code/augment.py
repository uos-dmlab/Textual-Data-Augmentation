...
def gen_eda(input_file, output_file, alpha_sr, alpha_ri, alpha_rs, alpha_rd, num_aug=9):
    ...
    for label, sentence in input_data:
        assert len(sentence)>0
        aug_sentences = eda(sentence, alpha_sr=alpha_sr, alpha_ri=alpha_ri, alpha_rs=alpha_rs, p_rd=alpha_rd, num_aug=num_aug)
        for aug_sentence in aug_sentences:
            if len(aug_sentence)>0:
                results.append((label, aug_sentence))

    with open(output_file, 'wb') as f:
        pickle.dump(results, f)
    print("generated augmented sentences with eda for " + input_file + " to " + output_file + " with num_aug=" + str(num_aug))
